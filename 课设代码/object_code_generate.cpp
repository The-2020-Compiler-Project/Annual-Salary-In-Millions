...
OPERAND RDL;
int table_flag = 1; //用该变量来标记需要加标号的位置 ,初始赋值为1来实现对开始处进行标号

int fcode::ADDR = 0;
vector<fcode> fcode_arrary; //存放生成的目标指令
vector<fcode> fcode_dseg_array; //数据段指令
...
void fcode::to_code()
{
    ...
    fcode_dseg_array.push_back(f);
    //中间填写定义的数据,对符号表扫一遍对变量分配存储单元

    //反填符号表中的偏移地址
    refill_addr();
    //根据偏移地址确定分配的数据段的大小;
    f.add_not_label("BUFF", "DB " + to_string(current_offset) + " DUP(0)", "");
    fcode_dseg_array.push_back(f);
    ...
    for (auto it = QT.begin(); it != QT.end(); it++) {
        ...
        //加，减，乘，除 ，与，或
            ...
            if (temp.sign == SIGN::add || temp.sign == SIGN::sub) {
                ...
                temp.sign == SIGN::multi ? op_temp = "IMUL" : op_temp = "IDIV";
            } else {
            ...
            if (RDL.position != temp.operand_1.position) //寄存器没被占用
            {
                ...
                f1.add("MOV", "AX", get_addr_of_operand(temp.operand_1));
                fcode_arrary.push_back(f1);
                if (op_temp == "IMUL" || op_temp == "IDIV") {
                    fcode f3;
                    f3.add("MOV", "BX", get_addr_of_operand(temp.operand_2));
...
{
    string result;
    for (unsigned i = 0; i < fcode_dseg_array.size(); i++) {
        result += print_fcode(fcode_dseg_array[i]);
    }
    for (unsigned i = 0; i < fcode_arrary.size(); i++) {
        result += print_fcode(fcode_arrary[i]);
    }
    return result;
}

string fcode::print_fcode(fcode f)
{
    string result = f.OP1 + ' ';
    if (f.OP2 != "") {
        result += f.OP2;
