    ...
    while(buffer!= '#')
        {
            ...
            {
                t[token_n].token_value+= buffer;
            }
            else if(s_now == end_s)		//到终止状态
            {
                t[token_n].token_code = token_code(s_before,t[token_n].token_value);//生成token序列码
                kind=token_kind(s_before,t[token_n].token_value);
                cout<<t[token_n].token_value<<"   <";
                if(kind==iT)
                    cout<<"iT ";
                else if(kind==cT)
                    cout<<"cT ";
                    ...
                    cout<<"PT ";
                cout<<t[token_n].token_code<<">"<<endl;
                fseek(f,-1,SEEK_CUR);
        ...
        cout<<t[token_n].token_code<<">"<<endl;
        token_n++;
        t[token_n].token_code = 111; //存入待识别符号串结束标记#
        t[token_n].token_value = '#';
        cout<<t[token_n].token_value<<"   <"<<t[token_n].token_code<<">"<<endl;
    return true;
...
int lexic::token_code(int s_before,string value)
{
    ...
    else if((s_before == 5)||(s_before == 3)) token_code = s_T_num++;
    else if(s_before == 8) token_code = i_T_num++;
    else if(s_before == 11) token_code = c_T_num++;
...
int lexic::token_kind(int s_before,string value)//返回token所处的类型
{
    if(s_before == 2)
    {
       return KT;
